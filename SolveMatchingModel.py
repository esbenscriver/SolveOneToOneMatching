...
# Import JAX code to solve a one-to-one matching model with transferable utility
...
def UpdateTransfer(t_init: jnp.ndarray, 
                   ...
                   nY: jnp.ndarray, 
                   probX: function, 
                   probY: function) -> tuple[jnp.ndarray, jnp.ndarray]:
...
def EndogenousVariables(cX: jnp.ndarray, 
  ...
  class Endog(Pytree, mutable=True):
    ...
    transfer = FixedPointRoot(fxp, transfer_init, acceleration=exog.acceleration)[0]
