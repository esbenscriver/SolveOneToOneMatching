...
class LogitModel(Pytree, mutable=False):
    ...
    def ChoiceProbabilities(self, v: jnp.ndarray) -> tuple[jnp.ndarray, jnp.ndarray]:
        ...
        expV_outside = jnp.where(
            self.outside_option, jnp.exp(-v_max), jnp.zeros_like(v_max)
        )
...
class NestedLogitModel(Pytree, mutable=False):
    ...
    def nest_structure(self) -> jnp.ndarray:
        ...
        nesting_structur = jnp.expand_dims(
            self.nest_index, self.axis
        ) == jnp.expand_dims(index_of_nests, 1 - self.axis)
...
class GeneralizedNestedLogitModel(Pytree, mutable=False):
    ...
    def ChoiceProbabilities(self, v: jnp.ndarray) -> tuple[jnp.ndarray, jnp.ndarray]:
        ...
        nominator_ni_k = jnp.einsum("nj, jk -> njk", expV_inside, self.nest_share) ** (
            1 / nest_parameter
        )
