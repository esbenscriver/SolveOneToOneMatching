...
nest_parameter_X = random.uniform(
    key=random.PRNGKey(211), shape=(types_X, nests_Y), minval=0.1, maxval=1.0
)
nest_parameter_Y = random.uniform(
    key=random.PRNGKey(212), shape=(types_Y, nests_X), minval=0.1, maxval=1.0
)

# Simulate nesting structure of the generalized nested logit model
nest_share_X = random.dirichlet(
    key=random.PRNGKey(311), alpha=jnp.ones((nests_Y,)), shape=(types_Y,)
)
