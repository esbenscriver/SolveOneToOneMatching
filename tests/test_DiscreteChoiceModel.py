...
def assert_model_outside_option_True(model_X: ModelType, model_name: str) -> None:
    ...
    demand_inside = jnp.sum(model_X.Demand(model_X.utility), axis=model_X.axis, keepdims=True)
    ...
    assert jnp.all(choice_probabilities_outside > 0.0), f"{model_name}: Choice probabilities (outside) less than zero: {jnp.min(choice_probabilities_outside) = }"
    assert jnp.all(choice_probabilities_outside < 1.0), f"{model_name}: Choice probabilities (outside) bigger than one: {jnp.max(choice_probabilities_outside) = }"

    assert jnp.all(choice_probabilities_inside_sum < 1.0), f"{model_name}: Sum of choice probabilities bigger than one: {jnp.max(choice_probabilities_inside_sum) = }"
    ...
    assert jnp.allclose(check_demand_sum, 0.0), f"{model_name}: Sum of demand does not sum to n: {jnp.min(check_demand_sum) = }, {jnp.max(check_demand_sum) = }"
...
def assert_model_outside_option_False(model_X: ModelType, model_name: str) -> None:
    ...
    assert jnp.allclose(choice_probabilities_outside, 0.0), f"{model_name}: Sum of choice probabilities of outside option is not zero: {jnp.min(choice_probabilities_outside) = }, {jnp.max(choice_probabilities_outside) = }"
    assert jnp.allclose(choice_probabilities_inside_sum, 1.0), f"{model_name}: Sum of choice probabilities does not sum to one: {jnp.min(choice_probabilities_inside_sum) = }, {jnp.max(choice_probabilities_inside_sum) = }"

    assert jnp.allclose(demand_inside, model_X.n), f"{model_name}: Sum of demand does not sum to n: {jnp.min(demand_inside) = }, {jnp.max(demand_inside) = }"
